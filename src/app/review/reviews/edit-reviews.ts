...
export class EditReviews {
    ...
    constructor(protected http: Http, protected toastr: ToastsManager, protected rconfig: ReviewConfig, private activatedRoute: ActivatedRoute) {
    ...
    load_review(id) {
        ...
        this.http.get(target).subscribe((res) => {
            ...
            this.month_years = {
                s_m: moment(this.review.starting_date).format('MM'),
                s_y: moment(this.review.starting_date).format('YYYY'),
                e_m: moment(this.review.end_date).format('MM'),
                e_y: moment(this.review.end_date).format('YYYY'),
            }; 
    ...
    save_start_end_date(v, op = 1) {

        switch (op) {
            case 1:
                this.review.starting_date = this.month_years.s_m + "-" + v + "-01";
                break;
            case 2:
                this.review.starting_date = v + "-" + this.month_years.s_m + "-01";
                break;
            case 3:
                this.review.end_date = this.month_years.e_y + "-" + v + "-01";
                break;
            case 4:
                this.review.end_date = v + "-" + this.month_years.e_m + "-01";
                break;
        }
        ...
        selected_reviewer.forEach(function (value, index) {
            ...
            if (managers.length > 0 || peers.length > 0) {
                ...
                if(value.template_id > 0)
                    r.template_id=value.template_id;
    ...
    loadEmployeesByDepartments(id) {
        ...
        this.http.get(target_url).subscribe((res) => {
            ...
            // evaluation multipl enteries........//
            /*

            if (this_obj.review.evaluation_type == 'on')
                emps.employees.forEach(function (value, index) {

                    var input_i = `<input type="radio" id="peer_id_dd" checked="checked"  name="employee_id" value="${value.id}">`;
                    this_obj.load_manager_peers(input_i, value);

                    emps.employees.forEach(function (v, k) {
                        this_obj.save_add_employee(v, 2, value.id);
                    });
